...
--%>
            ...
            <form id="classifyForm" action="<c:url value="${submitAction}" />" method="POST" onsubmit="return controller('${submitAction}', $(this).serialize());">
                ...
                <div id="synonyms">
                    ...
                    <c:forEach var="synonym" items="${symbolManager.getSymbolSynonyms(symbol.id)}" varStatus="iter">
                        <a href="#!nav=classify&sy=${synonym.id}">${synonym.name}</a><c:if test="${!iter.last}">,&nbsp;</c:if>
                    </c:forEach>  
                </div>
                <select id="selectedSynonym" name="selectedSynonym" size="7">
                        <c:forEach var="symbol" items="${projectManager.getProjectSymbols(project.id)}">
                ...
                <input type="submit" name="save" value="<fmt:message key="save" />" id="save" class="button" />
            </form>
            <c:choose>
